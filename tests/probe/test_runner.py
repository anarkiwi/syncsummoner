...
class FakePairCapture(FakeCapture):
    """Serves a native buffer alongside each RGB frame, as a YVYU-mode Capture does."""
    ...
    def read_pair(self):
        ...
        if frame is None:
            return None, None
        return (np.full((2, 2, 2), len(self.frames), dtype=np.uint8), frame)


class FakeWriter:
    """Collects what an open ``FrameWriter`` would have encoded."""

    def __init__(self):
        """init  ."""
        self.rows = []

    def write(self, frame, *, params, setpoint, loop_index=None, captured=None):
        """Write."""
        del loop_index, captured
        self.rows.append((frame, params, setpoint))
        return len(self.rows) - 1


def test_run_plan_archives_the_native_frames_it_measured():
    """Metrics and archive must describe the same frames, tagged with the same vector."""
    frames = [tagged(state, seed=10 * state + i) for state in range(3) for i in range(2)]
    writer = FakeWriter()
    _, records = run(FakePairCapture(frames), plan(3), archive=writer)
    assert len(records) == 3
    assert len(writer.rows) == 6, "every measured frame is archived"
    assert [row[2] for row in writer.rows] == [0, 0, 1, 1, 2, 2]
    assert {row[1] for row in writer.rows} == {r.params for r in records}
    assert all(row[0].dtype == np.uint8 for row in writer.rows)


def test_run_plan_without_an_archive_never_asks_for_native_frames():
    """The default path stays on ``read``; only archiving needs the card's own buffer."""

    class NoNative(FakeCapture):
        """Capture that fails loudly if the runner reaches for a native buffer."""

        def read_pair(self):
            """Read pair."""
            raise AssertionError("read_pair must not be called without an archive")

    frames = [tagged(state, seed=10 * state + i) for state in range(2) for i in range(2)]
    _, records = run(NoNative(frames), plan(2))
